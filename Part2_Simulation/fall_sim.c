...
typedef struct { double theta, extra_mag_scale, lin[3], extra_gyro[3]; double omega; } sample_t;

static FallDetector fd;
static int fa[3], fg[3];
static unsigned t_ms;
static int events[16];

static const char *ev_name[] = {"NONE","READY","FREEFALL","IMPACT","FALL_CONFIRMED","NEAR_FALL","RECOVERED","ACK","LONG_LIE","SOS"};
...
static void feed(double theta, double omega_dps, double grav_scale, double lx, double ly, double lz, double gx_extra, double gy_extra, int verbose)
{
    ...
    FallEvent e = FallDetector_Update(&fd, fa, fg, t_ms);
    if (e != FALL_EVENT_NONE) {
        events[e]++;
        if (verbose) {
            printf("    %6.2fs %-18s |a|=%4d |w|=%4d tilt=%3d", t_ms / 1000.0, ev_name[e], fd.accel_mag_mg, (fd.gyro_mag_mdps/1000), fd.tilt_deg);
            if (e == FALL_EVENT_FALL_CONFIRMED || e == FALL_EVENT_NEAR_FALL)
                printf("  rot=%d posture=%d still=%d%% %s", (fd.last_rotation_mdps/1000), fd.last_posture_deg, fd.last_inactive_pct, fd.last_reason);
            printf("\n");
        }
    }
    t_ms += 20;
...
static void reset(void) { memset(events, 0, sizeof events); memset(fa, 0, sizeof fa); memset(fg, 0, sizeof fg); t_ms = 0; th = 0; FallDetector_Init(&fd, 0); hold(2.0, 0); }
...
static int run(const char *name, void (*scenario)(int), int expect_fall, int reps)
{
    ...
    for (int r = 0; r < reps; r++) {
        ...
        reset();
        scenario(r == 0);
        int fall = events[FALL_EVENT_FALL_CONFIRMED] > 0;
...
int main(void)
{
    ...
    printf("  long lie after 30 s: %s\n", fd.phase == FALL_PHASE_LONG_LIE ? "OK" : "FAIL");
    total++; pass += (fd.phase == FALL_PHASE_LONG_LIE);
    FallEvent e = FallDetector_Acknowledge(&fd, t_ms);
    printf("  button ack clears long lie: %s\n", (e == FALL_EVENT_ACKNOWLEDGED && fd.phase == FALL_PHASE_MONITORING) ? "OK" : "FAIL");
    total++; pass += (e == FALL_EVENT_ACKNOWLEDGED && fd.phase == FALL_PHASE_MONITORING);

    srand(6); reset(); s_fall_freefall(0); hold(1, 0); rotate(-90, 1.0, 1, 1); hold(4, 1);
    printf("  self-recovery after getting up: %s\n", fd.phase == FALL_PHASE_MONITORING ? "OK" : "FAIL");
    total++; pass += (fd.phase == FALL_PHASE_MONITORING);

    srand(7); reset(); s_fall_freefall(0); hold(5, 0);
    printf("  fall indication stays latched while lying (5 s): %s\n", fd.phase == FALL_PHASE_ALARM ? "OK" : "FAIL");
    total++; pass += (fd.phase == FALL_PHASE_ALARM);

    srand(8); reset(); e = FallDetector_RequestSOS(&fd, t_ms); hold(3, 0);
    int sos_ok = (e == FALL_EVENT_SOS && fd.phase == FALL_PHASE_SOS);
    e = FallDetector_Acknowledge(&fd, t_ms);
    printf("  SOS via long press, cleared by short press: %s\n", (sos_ok && e == FALL_EVENT_ACKNOWLEDGED) ? "OK" : "FAIL");
    total++; pass += (sos_ok && e == FALL_EVENT_ACKNOWLEDGED);
